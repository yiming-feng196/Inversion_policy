...
def main() -> None:
    ...
    steps_per_epoch = args.max_train_steps if args.max_train_steps else natural_steps
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.max_train_steps:
            required = args.max_train_steps * args.batch_size
            passes = math.ceil(required / len(train_indices))
            order = torch.cat([
                train_indices[torch.randperm(
                    len(train_indices),
                    generator=torch.Generator().manual_seed(args.seed + epoch * 10_000 + pass_index),
                )]
